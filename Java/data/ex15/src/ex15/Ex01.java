...
public class Ex01 {
	...
	public static void main(String[] args) {
		...
		ArrayList<Ex01User> arruser = new ArrayList<>();
			...
			switch (menuNum) {
				case 1:
					if (isbank) {
						...
						user.setUserPw(scanner.nextLine());
						// 완료
					}
					break;
				case 2:
					...
					if (isbank) {
						...
						for (int i = 0; i < transferNum; i++) {
							if (arrTransfer[i].name.equals(arrUser[index].userName))
								System.out.println(arrTransfer[i].status + "\t" + arrTransfer[i].money + "\t"
										+ arrTransfer[i].otherName);
						}
						System.out.println();
					} else if (isVendingMc) {
						System.out.println("없는번호");
